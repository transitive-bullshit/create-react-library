...
module.exports.copyTemplateFile = async (opts) => {
  ...
  const fileTemplate = handlebars.compile(fileName, 'utf8')
  const destFileName = fileTemplate({shortName: info.shortName})

  await mkdirp(destFileDir)
  fs.writeFileSync(path.join(destFileDir, destFileName), content, 'utf8')
